...
const Login = () => {
  ...
  const loginWithGoogle = async () => {
    ...
    if (user) {
      ...
      const secondToLastPath =
        previousPath?.length >= 2
          ? previousPath[previousPath.length - 2]
          : null;
      console.log("🚀 ~ loginWithGoogle ~ secondToLastPath:", secondToLastPath);
  ...
  const handleResetPassword = async () => {
    try {
      ...
      setMessage(
        "Thanks! If we find a matching account, we'll send you an email. Please check your inbox."
      );
    } catch (error) {
        ...
        <div className="container mx-auto px-4">
          {resetPassword ? (
            <div className="max-w-[499px] relative m-auto bg-white md:p-[50px] p-5 rounded-[20px]  shadow-[0_15px_30px_rgba(0,0,0,0.05)]">
              <RxCross1
                className="text-white text-2xl absolute -top-10 cursor-pointer right-3"
                onClick={goBack}
              />
              ...
              {message ? (
                <p className="text-lg font-normal text-center text-darkGreen">
                  <span className="text-darkBlue">✔ </span> {message}
                </p>
              ) : (
                <div>
              ...
              <button
                className="text-center mt-5 w-full text-blue-500 hover:underline"
            ...
            <div className="max-w-[499px] relative m-auto bg-white md:p-[50px] p-5 rounded-[20px]  shadow-[0_15px_30px_rgba(0,0,0,0.05)]">
              ...
              <p className="text-lg font-normal text-darkBlue/50 text-center">
                ...
                <Link href="/login" className="font-medium text-lightBlue">
                  Register
                </Link>{" "}
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="mt-7">
                <div className="grid gap-4">
                  ...
                  {errorMessage && (
                    <span className="text-center text-red-400">
                      {errorMessage?.message?.includes("user-not-found") ? (
              ...
              <button
                ...
                className="text-lg -mt-2 font-medium px-[20px] py-[14px] bg-blue-500 text-white hover:bg-white hover:text-blue-500 border border-blue-500 rounded-[40px] w-full"
              >
                Login With Google
              </button>
              <button
                type="submit"
                className="text-lg mt-3 font-medium px-[20px] py-[14px] bg-gray-800 text-white hover:bg-white hover:text-gray-800 border border-gray-800 rounded-[40px] w-full"
              >
