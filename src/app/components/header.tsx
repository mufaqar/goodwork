...
const Header = () => {
    ...
    return (
        <>
                        ...
                        <ul className={`flex md:flex-row flex-col gap-7 ${open ? "absolute top-[90px] left-0 h-screen w-screen bg-darkBlue py-8 px-4" : "md:flex hidden"} transition-transform duration-300 ease-in-out `}>
                            {NavLinks.map((item: NavLinksType, idx: number) => {
                                return <li key={idx}>
                                    ...
                                    <ul className={` flex-col md:absolute md:px-5 md:pb-5 pb-0 pt-5 2xl:top-[81px] top-[71px] gap-4 bg-darkBlue ${dropdown === item.id ? 'flex' : 'hidden'} `}>
            ...
            <div onClick={() => { setDropdown(null), setOpen(false) }} className={`${open || dropdown ? 'block z-[1]' : 'block -z-30'} bg-red-800 opacity-0  absolute top-0 bottom-0 left-0 right-0 `}></div>
        </>
    )
}
